...
def handle_docx(file_path):
    ...
    folder = "output_docx"
    log_info(f"📄 Handling DOCX file: {filename}")
    ...
    try:
        ...
        if text.strip():
            ...
            try:
                save_text_as_txt(text, out_path)
                log_info(f"✅ DOCX text saved to: {out_path}")
                print(f"✅ Saved to: {out_path}")
            except Exception as save_err:
                log_error(f"❌ Error saving DOCX text for {filename}: {save_err}")
                return
            ...
            log_info(f"⚠️ No text extracted from DOCX: {filename}")
            print(f"⚠️ No text extracted from {filename}")

    except Exception as e:
        log_error(f"❌ DOCX read error for {filename}: {e}")
