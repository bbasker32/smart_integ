...
def handle_pptx(file_path):
    ...
    folder = "output_pptx"
    log_info(f"📄 Handling PPTX file: {filename}")
    ...
    try:
        ...
        if text.strip():
            ...
            try:
                ...
                log_info(f"✅ PPTX text saved to: {out_path}")
                print(f"✅ Saved to: {out_path}")
            except Exception as save_err:
                log_error(f"❌ Error saving PPTX text for {filename}: {save_err}")
                return
            ...
            log_info(f"⚠️ No text extracted from PPTX: {filename}")
            print(f"⚠️ No text extracted from {filename}")

    except Exception as e:
        log_error(f"❌ PPTX read error for {filename}: {e}")
